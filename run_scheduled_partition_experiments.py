...
NOISES = {
    "sc_1q2q": "sc_scheduled_1q2q",
    "sc_scheduled_1q2q": "sc_scheduled_1q2q",
    "sc_2q": "sc_scheduled_2q",
    "sc_scheduled_2q": "sc_scheduled_2q",
    "ion_1q2q": "ion_scheduled_1q2q",
    "ion_scheduled_1q2q": "ion_scheduled_1q2q",
    "ion_2q": "ion_scheduled_2q",
    "ion_scheduled_2q": "ion_scheduled_2q",
}

NOISE_PRESETS = {
    "all": ("sc_scheduled_1q2q", "sc_scheduled_2q", "ion_scheduled_1q2q", "ion_scheduled_2q"),
    ...
    "sc": ("sc_scheduled_1q2q", "sc_scheduled_2q"),
    "ion": ("ion_scheduled_1q2q", "ion_scheduled_2q"),
    "1q2q": ("sc_scheduled_1q2q", "ion_scheduled_1q2q"),
    "2q": ("sc_scheduled_2q", "ion_scheduled_2q"),
}
...
def expand_names(values, items, presets, kind):
    expanded = []
    for value in values:
        key = value.lower().replace("-", "_").replace(" ", "_").replace("&", "")
        if key in presets:
...
def noise_value(noise_key):
    return NOISES.get(noise_key, noise_key)
...
def result_file(output_dir, molecule_key, noise_key, circuit_key):
    ...
    return Path(get_file_name(prefix, noise_value(noise_key), CIRCUITS[circuit_key]))
...
def print_tasks(tasks, output_dir):
    for index, (molecule_key, noise_key, circuit_key) in enumerate(tasks, start=1):
        ...
        print(
            f"{index:03d}: {molecule_key} {noise_value(noise_key)} {CIRCUITS[circuit_key]} "
            f"-> {path} [{state}]"
        )
...
def run_tasks(args):
    ...
    for task_index, (molecule_key, noise_key, circuit_key) in enumerate(tasks, start=1):
        ...
        try:
            ...
            failures.append((molecule_key, noise, circuit_key, exc))
            LOGGER.exception("Failed: %s %s %s", molecule_key, noise, circuit_key)
            if not args.keep_going:
                raise

    if failures:
        LOGGER.error("%s task(s) failed", len(failures))
        for molecule_key, noise, circuit_key, exc in failures:
            LOGGER.error("%s %s %s: %s", molecule_key, noise, circuit_key, exc)
        return 1
    return 0
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Run scheduled hardware-noise partition experiments: 1q&2q and 2q depolarization."
    )
    ...
    parser.add_argument(
        "--noises",
        nargs="+",
        default=["scheduled_partitions"],
        help="Noise modes or presets: scheduled_partitions, sc, ion, 1q2q, 2q.",
    )
    parser.add_argument("--output-dir", default="data_scheduled_partitions")
